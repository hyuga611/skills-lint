/**
 * skills-lint used to treat an unrecognised `--flag` as a path to scan.
 *
 * That turned the linter off in silence. Measured against a repo with a real
 * finding in it: `skills-lint` exited 1, `skills-lint --zzz-not-a-flag` exited 0. Any
...
const empty = () => mkdtempSync(join(tmpdir(), 'skills-lint-cli-'));
...
test('--help prints usage and exits 0', () => {
  ...
  try {
    for (const flag of ['--help', '-h']) {
      ...
      assert.match(r.out, /^skills-lint \d+\.\d+\.\d+ /, `${flag} must name the tool and its version`);
